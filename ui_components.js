...
var LottoUI = {
    ...
    Chart: {
        ...
        _renderCurve: function(container, distData, unit, stat, config) {
            ...
            var scaleY = (h - padding - bottomSpace) / (Math.max.apply(null, points.map(p => p.y)) || 1);
            var getX = v => sidePadding + (v - minX) * scaleX;
            var getY = v => (h - bottomSpace) - v * scaleY;
            var pathD = "M " + points.map(p => getX(p.x).toFixed(1) + "," + getY(p.y).toFixed(1)).join(" L ");
            ...
            var svg = `<svg viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg" style="width:100%; height:100%; overflow:visible;">
                ...
                <path d="${pathD}" fill="none" stroke="#3182f6" stroke-width="3" stroke-linecap="round"/>
                <line x1="${sidePadding}" y1="${h-bottomSpace}" x2="${w-sidePadding}" y2="${h-bottomSpace}" stroke="#e5e8eb"/>
                ${[{v:mean-2*std, l:'2.5%'}, {v:mean, l:config.label}, {v:mean+2*std, l:'97.5%'}].map(l => {
                    var val = LottoUtils.round(Math.max(l.v, dataMin), unit==='개'?0:1);
                    return `<g><text x="${getX(l.v)}" y="${h-bottomSpace+22}" text-anchor="middle" font-size="10" font-weight="900">${val}${unit}</text></g>`;
                }).join('')}
            </svg>`;
        ...
        markov: function(containerId, matrix, options) {
            ...
            if (!container) return;
            var opts = options || {};
            var html = '<div style="display:grid; grid-template-columns: 40px repeat(10, 1fr); gap: 2px;"><div></div>' + Array.from({length:10}, (_,i) => `<div style="text-align:center; font-size:0.65rem; font-weight:900; color:#8b95a1;">${i}</div>`).join('');
    ...
    Card: {
        combo: function(res, options) {
            ...
            var ballsHtml = res.nums.map(n => LottoUI.Ball.create(n, { mini: true }).outerHTML).join('');
    ...
    Feedback: {
        toast: function(message, actionLabel, actionCallback) {
            ...
            if (actionLabel && actionCallback) {
                ...
                btn.onclick = () => { actionCallback(); container.remove(); };
                container.appendChild(btn);
            }

            document.body.appendChild(container);
            setTimeout(() => { if(container.parentNode) { container.style.animation = 'fadeOutDown 0.3s forwards'; setTimeout(() => container.remove(), 300); } }, 4000);
        },
        tooltip: function(target, text) {
            ...
            target.addEventListener('mouseleave', () => { if(target._tip) { target._tip.remove(); target._tip = null; } });
        }
    }
};
