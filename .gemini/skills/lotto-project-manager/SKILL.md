...
# Lotto Project Manager Skill (v15.1)
...
2.  **구현 원칙 (v15.1+)**:
    *   **아키텍처 분리 (Modular Hub)**: `core.js`는 공통 인프라를, `unified_engine.js`는 핵심 비즈니스 로직을, `ui_components.js`는 순수 UI 렌더링을 담당한다.
    *   **엄격한 로드 순서**: 모든 HTML 파일은 `indicators.js` -> `core.js` -> `unified_engine.js` -> `pension_utils.js`(연금 한정) -> `ui_components.js` -> `engine.js` 순으로 로드하여 종속성 오류를 방지한다.
    *   **이벤트 바인딩 무결성**: 페이지별 전용 엔진(`engine.js`)은 `init` 단계에서 필요한 모든 UI 이벤트를 누락 없이 바인딩해야 하며, 사용되지 않는 구형 버튼 ID(Legacy IDs)는 즉시 제거하여 혼선을 차단한다.
    ...
    *   **버전 동기화 (Vibe Sync)**: 시스템 버전 갱신 시 `node sync_version.cjs`를 실행하여 모든 HTML 리소스 파라미터와 문서 버전을 일괄 동기화한다. (현재 v15.1 표준)
