...
# Lotto Project Manager Skill (v29.4)
...
2.  **구현 원칙 (v29.4+)**:
    ...
    *   **Adaptive Visualization**: 모든 통계 차트는 `viewBox` 기반의 반응형 SVG로 구현하며, `IntersectionObserver`를 통한 지연 렌더링을 적용한다.
    *   **Data-Driven Clamping**: 차트 라벨 등 시각화 요소는 이론적 통계값($\mu \pm 2\sigma$)이 실제 데이터의 최소/최대 범위를 벗어날 경우, **실제 데이터의 경계값**을 우선하여 표시함으로써 사용자 혼란을 방지한다.
    *   **SSOT (Single Source of Truth)**: 모든 지표와 예측 전략 설정은 `indicators.js`에서 통합 관리한다.
    *   **Logic Mirroring**: Python 백엔드 데이터와 JS 프론트엔드 연산 결과는 항상 100% 일치해야 한다.
