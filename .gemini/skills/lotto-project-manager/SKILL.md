...
1.  **사전 검토 (Pre-Check)**:
    ...
    *   **백테스트 기반 검증**: 새로운 시너지 규칙이나 필터 추가 시, 반드시 역대 데이터를 통한 백테스트를 수행하여 오탐지율을 확인한다. (오탐지 0% 항목은 `danger` 등급 부여)
...
2.  **구현 원칙**:
    ...
    *   **25/5/15 전략**: AI 예측 번호 풀은 '추천 30 / 보류 5 / 제외 10' 구성을 표준으로 유지한다.
    *   **차별화된 감점**: 조합 분석 시 등급별 감점 폭(Danger -25, Warning -10)을 엄격히 준수한다.
