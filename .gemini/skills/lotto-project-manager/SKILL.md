...
2.  **구현 원칙**:
    ...
    *   **30/5/10 전략**: AI 예측 번호 풀은 시뮬레이션 최적 수치인 '추천 30 / 보류 5 / 제외 10' 구성을 표준으로 유지한다.
    *   **UI 그리드 표준**: 예측 번호 풀은 가독성을 위해 반드시 10열 그리드 레이아웃을 준수한다.
    *   **데이터 밀착형 레이아웃**: 통계 화면은 차트와 데이터 표의 시선 분산을 최소화하기 위해 밀착형 레이아웃(v9.8)을 적용한다.
