...
# Lotto Project Manager Skill (v14.8)
...
2.  **구현 원칙 (v14.8+)**:
    *   **아키텍처 분리 (Modular Hub)**: `core.js`는 공통 비즈니스 로직을, `pension_utils.js`는 연금 전용 로직을, `ui_components.js`는 순수 UI 렌더링(`LottoUI`)을 담당한다.
    *   **엄격한 로드 순서**: 모든 HTML 파일은 `indicators.js` -> `core.js` -> `pension_utils.js`(연금 한정) -> `ui_components.js` -> `engine.js` 순으로 로드하여 종속성 오류를 방지한다.
    *   **정밀 레이아웃 표준 (Expert Table)**: 모든 통계 표는 콘텐츠 밀착형 컬럼 너비(회차 55px, 당첨번호 230px, 결과값 80px)와 중앙 정렬을 필수 적용하여 시각적 정합성을 유지한다.
    *   **AI 전략 표준 (10-Strategies)**: 추천 시스템은 `prediction.js`의 10대 핵심 전략(다차원, 회귀, 이웃수 등)에 기반하여 전략당 1개씩 총 10개의 고유 조합을 생성한다.
    *   **컴포넌트 기반 UI (LottoUI)**: 모든 구슬, 카드, 배지 등은 `ui_components.js`를 통해 생성하며, 인라인 스타일 사용을 지양하고 전역 CSS 클래스를 활용한다.
    *   **버전 동기화 (Vibe Sync)**: 시스템 버전 갱신 시 `node sync_version.cjs`를 실행하여 모든 HTML 리소스 파라미터와 문서 버전을 일괄 동기화한다. (현재 v14.8 표준)
