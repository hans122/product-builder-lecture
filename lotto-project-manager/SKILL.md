...
# Lotto Project Manager Skill (v16.2)
...
1.  **사전 검토 (Pre-Check)**:
    ...
    *   **Safe Context Calling**: 유틸리티 함수(LottoUtils)를 `filter`, `map` 등 콜백으로 전달할 때는 `this`를 지양하고 `LottoUtils.method()`와 같이 명시적으로 호출하여 컨텍스트 유실(`TypeError`)을 방지한다.
...
2.  **구현 원칙 (v16.2+)**:
    ...
    *   **버전 동기화 (Vibe Sync)**: 시스템 버전 갱신 시 `node sync_version.cjs`를 실행하여 모든 HTML 리소스 파라미터와 문서 버전을 일괄 동기화한다. (현재 v16.2 표준)
...
*   "콜백 함수 활용" -> `nums.filter(n => LottoUtils.isPrime(n))` 형식을 사용하여 `this` 에러를 사전에 차단한다.
