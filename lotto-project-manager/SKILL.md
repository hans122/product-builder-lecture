...
# Lotto Project Manager Skill (v13.5)
...
2.  **구현 원칙 (v13.5+)**:
    ...
    *   **스켈레톤 로딩 (Skeleton First)**: 데이터 로딩 중에는 `LottoUI.showSkeletons`를 통해 사용자 경험을 유지한다.
    *   **버전 동기화 (Vibe Sync)**: 시스템 버전 갱신 시 `node sync_version.cjs`를 실행하여 모든 HTML 리소스 파라미터와 문서 버전을 일괄 동기화한다. (현재 v13.5 표준)
