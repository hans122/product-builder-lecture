...
2.  **구현 원칙**:
    ...
    *   **차트 시각화 표준**: 모든 분포 차트(Curve Chart)는 정규분포 기반 백분위(0%, 2.5%, 16%, 50%, 84%, 97.5%, 100%) 라벨을 준수하며, 라벨 겹침 방지(Collision Detection) 로직을 필수로 적용한다.
    *   **리포트 최적화**: 연금 분석 리포트는 최근 5회차로 제한하여 모바일 가독성을 확보한다.
