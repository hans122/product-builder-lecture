...
def analyze_lotto():
    ...
    for i, draw in enumerate(draws):
        ...
        # [v32.0] 번호 간 최대 간격 계산
        m_gap_val = max([nums_list[j+1] - nums_list[j] for j in range(5)]) if len(nums_list)==6 else 0

        streak_2 = prev_1.intersection(set(draws[i-2]["nums"] if i>1 else []))
        ind_s = len([n for n in nums_list if n in streak_2])
        ...
        color_indices = [(n-1)//10 for n in nums_list]; c_counts = Counter(color_indices)
        
        m = {
            "sum": sum(nums_list), "odd_count": len([n for n in nums_list if n%2!=0]), "low_count": len([n for n in nums_list if n<=22]),
            "period_1": len([n for n in nums_list if n in prev_1]), "period_2": len([n for n in nums_list if i>1 and n in draws[i-2]["nums"]]), "period_3": len([n for n in nums_list if i>2 and n in draws[i-3]["nums"]]), "neighbor": len([n for n in nums_list if any(abs(n-px)==1 for px in (draws[i-1]["nums"] if i>0 else []))]), "consecutive": sum(1 for j in range(5) if nums_list[j]+1 == nums_list[j+1]), "prime": len([n for n in nums_list if is_prime(n)]), "composite": len([n for n in nums_list if is_composite(n)]), "multiple_3": len([n for n in nums_list if n%3==0]), "multiple_4": len([n for n in nums_list if n%4==0]), "square": len([n for n in nums_list if n in [1,4,9,16,25,36]]), "double_num": len([n for n in nums_list if n in [11,22,33,44]]), "mirror": len([n for n in nums_list if n in mirs]), "bucket_15": len(set((n-1)//15 for n in nums_list)), "bucket_9": len(set((n-1)//9 for n in nums_list)), "bucket_7": len(set((n-1)//7 for n in nums_list)), "bucket_5": len(set((n-1)//5 for n in nums_list)), "p9": len(set((n-1)%9 for n in nums_list)), "empty_zone": [0,0,0,0,0].count(0), "color": len(c_counts), "max_same_color": max(c_counts.values()), "pattern_corner": len([n for n in nums_list if n in corners]), "pattern_center": len([n for n in nums_list if n in tri_c]), "end_sum": sum(n%10 for n in nums_list), "same_end": max(Counter([n%10 for n in nums_list]).values()), "ac": calculate_ac(nums_list), "span": nums_list[-1]-nums_list[0], "mean_gap": round((nums_list[-1]-nums_list[0])/5, 1), "individual_streak": ind_s, "over_appearance": over_app, "recent_5_recurrence": r5_rec, "hot_10_count": h10, "cold_20_count": c20, "avg_recurrence_interval": avg_int, "max_gap": m_gap_val
        }
