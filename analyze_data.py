...
def analyze_lotto():
    ...
    for i, draw in enumerate(draws):
        ...
        # v32.7 멸구간 정밀 계산 (1-10, 11-20, 21-30, 31-40, 41-45)
        zones = [0, 0, 0, 0, 0]
        for n in nums_list:
            z_idx = (n - 1) // 10
            if z_idx > 4: z_idx = 4
            zones[z_idx] += 1
        ez_val = zones.count(0)
        ...
        r5_draws = draws[max(0, i-5):i]; r5_rec = 0
        if r5_draws:
            all_r5 = [n for d in r5_draws for n in d["nums"]]
            r5_rec = sum(Counter(all_r5)[n] for n in nums_list)
        ...
        m = {
            "sum": sum(nums_list), "odd_count": len([n for n in nums_list if n%2!=0]), "low_count": len([n for n in nums_list if n<=22]),
            "period_1": len([n for n in nums_list if n in prev_1]), "period_2": len([n for n in nums_list if i>1 and n in draws[i-2]["nums"]]), "period_3": len([n for n in nums_list if i>2 and n in draws[i-3]["nums"]]), "neighbor": len([n for n in nums_list if any(abs(n-px)==1 for px in prev_1)]), "consecutive": sum(1 for j in range(5) if nums_list[j]+1 == nums_list[j+1]), "prime": len([n for n in nums_list if is_prime(n)]), "composite": len([n for n in nums_list if is_composite(n)]), "multiple_3": len([n for n in nums_list if n%3==0]), "multiple_4": len([n for n in nums_list if n%4==0]), "square": len([n for n in nums_list if n in [1,4,9,16,25,36]]), "double_num": len([n for n in nums_list if n in [11,22,33,44]]), "mirror": len([n for n in nums_list if n in [12,21,13,31,14,41,23,32,24,42,34,43]]), "bucket_15": len(set((n-1)//15 for n in nums_list)), "bucket_9": len(set((n-1)//9 for n in nums_list)), "bucket_7": len(set((n-1)//7 for n in nums_list)), "bucket_5": len(set((n-1)//5 for n in nums_list)), "p9": len(set((n-1)%9 for n in nums_list)), "empty_zone": ez_val, "color": len(c_counts), "max_same_color": max(c_counts.values()), "pattern_corner": len([n for n in nums_list if n in [1,2,8,9,6,7,13,14,29,30,36,37,34,35,41,42]]), "pattern_center": len([n for n in nums_list if n in [17,18,19,24,25,26,31,32,33]]), "end_sum": sum(n%10 for n in nums_list), "same_end": max(Counter([n%10 for n in nums_list]).values()), "ac": calculate_ac(nums_list), "span": nums_list[-1]-nums_list[0], "mean_gap": round((nums_list[-1]-nums_list[0])/5, 1), "individual_streak": ind_s, "over_appearance": len([n for n in nums_list if i>5 and Counter([n for d in draws[i-5:i] for n in d["nums"]])[n]>=3]), "recent_5_recurrence": r5_rec, "hot_10_count": h10, "cold_20_count": c20, "avg_recurrence_interval": avg_int, "max_gap": m_gap_val
        }
...
def analyze_pension():
    ...
    print(f"✅ Pension Analysis Complete (v32.7): {len(draws)} draws.")
