    ...
    document.getElementById('p-analyze-btn').onclick = function() {
        ...
        resultsArea.innerHTML = 
            ...
            '<div class="pos-chart-box" style="margin-top: 20px; padding: 20px; background: #fffdfa; border: 1px solid #f1e6d0;">' +
                '<h4 style="margin-top: 0; color: #856404;">📝 통계적 분석 의견</h4>' +
                '<p style="font-size: 0.85rem; color: #475569; line-height: 1.6; margin: 0;">' +
                    '선택하신 <b>' + state.group + '조 ' + digits.join('') + '</b> 조합은 ' +
                    (grade === 'S' ? '역대 당첨 데이터의 골든 존에 완벽하게 부합하는 최상의 밸런스입니다.' : '통계적으로 무난한 구성을 가지고 있습니다.') +
                    (pattern.maxOccur >= 3 ? ' 다만, 특정 숫자가 3회 이상 중복되는 패턴은 다소 인위적일 수 있으니 유의하시기 바랍니다.' : '') +
                '</p>' +
            '</div>';
